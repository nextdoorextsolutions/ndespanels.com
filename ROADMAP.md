...
### Phase A: Finance & Data Sync
- [ ] **Fix Finance Navigation:** Ensure "Home/Back" buttons route correctly to `/dashboard`.
- [ ] **Sync Finance Jobs:** Update the Finance page to query the MAIN jobs database (`trpc.jobs.getAll`).
...
- [ ] **Client Filter:** "Clients" page should ONLY show jobs in `Approved`, `Project Scheduled`, `Completed`, or `Invoiced` stages.

### Phase B: Permissions
- [ ] **Office Staff Access:** Update RBAC to give `office_staff` role full Read/Write/Send access to Invoicing (currently Admin/Owner only).
